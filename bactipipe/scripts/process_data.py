...
def make_summary(qc_summary, temp_qc_summary, header, checkm_out, logfile=None):
    ...
    with(open(qc_summary , 'w')) as qc_sum, open(temp_qc_summary, 'r') as temp_sum:
        ...
        writer.writerow([""])
        writer.writerow(["Sample",  "Mean_quality", "Expected organism", "Identified organism", "Percent Match", "Coverage", "CheckM completeness", "CheckM contamination", "Quality comment"])
        for line in temp_sum:
            ...
            if sample in checkm_out:
                ...
                contamination_verdict = "Pass" if float(contamination) <= 5 else "Fail"
            else:
            ...
            if any([qc_verdict == "Fail", cov_verdict == "Fail", tax_confirm == "Fail", tax_confirm == "N/A", completeness_verdict == "Fail", contamination_verdict == "Fail"]):
                ...
                final_verdict = "Pass"
            writer.writerow([sample, f'{float(avqc):.2f}', organism, hit, match, f'{float(coverage):.2f}', completeness, contamination, final_verdict])
